...
def generate_launch_description():
    ...
    # ========== 原有参数 ==========
    declare_uart_enable = DeclareLaunchArgument('uart_enable', default_value='True')
    ...
    declare_use_camera = DeclareLaunchArgument(
        ...
        description='是否启动本地大恒相机+编码器'
    )
    declare_separate_decoder = DeclareLaunchArgument(
        'enable_separate_decoder', default_value='False',
        description='是否开启各通道独立解码窗口'
    )

    declare_use_shark = DeclareLaunchArgument('use_shark', default_value='True')
    ...
    declare_use_mqtt = DeclareLaunchArgument('use_mqtt', default_value='True')
    declare_mqtt_ip = DeclareLaunchArgument('mqtt_server_ip', default_value='192.168.12.1')
    ...
    declare_debug_mode = DeclareLaunchArgument(
        ...
        description='调试模式：开启串口回传接收，解码后显示在QT的MQTT界面'
    )
    ...
    # MQTT裁判+视频（调试模式下自动关闭MQTT视频流，避免和串口回传冲突）
    ...
    # 调试模式：串口回流解码器 → 输出到 /decoded_image/local（QT第一路）
    ...
    qt_launch = IncludeLaunchDescription(
        ...
        condition=LaunchConfigurationEquals('use_qt_gui', 'True')
    )

    return LaunchDescription([
        # 参数声明
        declare_uart_enable, declare_uart_port, declare_uart_baud,
        declare_use_camera, declare_separate_decoder,
